...
NW.plugins.chapter = NW.plugins.chapter || {};
NW.plugins.chapter.editorStyle = {
...
// 迁移期兼容别名:未迁移组件仍用全局名引用本组件
const EditorStyle = NW.plugins.chapter.editorStyle;
window.EditorStyle = EditorStyle;

// 插件脚本由引导器动态加载,加载时 DOMContentLoaded 通常已过,须按 readyState 判断
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initEditorStyle);
} else {
  initEditorStyle();
}
